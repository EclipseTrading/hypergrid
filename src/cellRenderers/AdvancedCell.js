...
var AdvancedCell = CellRenderer.extend('AdvancedCell', {
    paint: function (gc, config) {
        ...
        // TODO: DISABLE VALUE CHECKING FOR NOW
        ...
        // Handling selection highligting
        if (true) {
        ...
        if (true) {
            ...
            // Test handle regex here
            if (config.regexFormat && config.regexFormat.length > 0) {
                for (let reg of config.regexFormat) {
                    let matchs = val.matchAll(reg.regex);
                    let xIndex = 0;
                    let regexX = x;
                    let regexWidth = width;
                    let consumedWidth;
                    let result = matchs.next();
                    if (result.done) {
                        // assume single
                        ...
                        // Setup new config
                        let newBgColor = reg.format.backgroundColor !== undefined ? [...colors, reg.format.backgroundColor] : colors;
                        let rgxConfig = { ...config, bounds: {...config.bounds} };
                        while (!result.done) {
